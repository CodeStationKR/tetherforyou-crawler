...
class OkxCrawler(BaseCrawler):
    def __init__(self, chrome_path, user_data_directory, profile_directory):
        self.base_url = f"https://www.okx.com/affiliates/invitees"
    ...
    def get_uid(self, tds):
        return tds[-2].find_element(By.CSS_SELECTOR, 'div > div').text.replace('\n', '').replace(' ', '')
    ...
    def get_settled_commission(self, tds):
        result = 0.0
        settled_commission = tds[-4].find_element(By.CSS_SELECTOR, 'div > span').text
